...
<SCRIPT>
	...
	function loadVendorQnaTable(startDate, endDate) {	//submit 되더라도 table페이지로 이동하지 않고 table이 해당페이지에 나타나도록 함
		var xhttp = new XMLHttpRequest();
        xhttp.open("POST", "vendorQnaTable.jsp", true);
...
<br /><br />
	<DIV style="display:inline-block; border:2px solid lightgray; padding:15px; width:850px; height:37px;">
		...
		<FORM style="display:inline-block" onsubmit="loadVendorQnaTable(document.getElementById('start').value, document.getElementById('end').value)); return false;">
			<INPUT type="date" id="start" name="start" />~
			<INPUT type="date" id="end" name="end" />&nbsp;&nbsp;
			<INPUT type="submit" value="조회" />
		</FORM>
	</DIV><br /><br />
<jsp:include page="vendorQnaTable.jsp" />