...
<SCRIPT>
	...
	function loadCancellationTable(startDate, endDate) {	//submit 되더라도 table페이지로 이동하지 않고 table이 해당페이지에 나타나도록 함
		var xhttp = new XMLHttpRequest();
		xhttp.open("POST", "cancellationTable.jsp", true);
		xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xhttp.send("start=" + startDate + "&end=" + endDate);
	}
</SCRIPT>
	
<DIV style="font-size:25px; font-weight:bold"> 취소/반품/교환 처리 현황 </DIV> <br />
	<DIV style="display:inline-block; border:2px solid lightgray; padding:15px; width:850px; height:37px;">
		...
		<FORM style="display:inline-block" onsubmit="loadCancellationTable(document.getElementById('start').value, document.getElementById('end').value)); return false;">
