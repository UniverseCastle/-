...
<SCRIPT>
	...
	function loadCancellationRequestTable(startDate, endDate) {	//submit 되더라도 table페이지로 이동하지 않고 table이 해당페이지에 나타나도록 함
		var xhttp = new XMLHttpRequest();
		xhttp.open("POST", "cancellationRequestTable.jsp", true);
...
<DIV style="font-size:25px; font-weight:bold"> 취소/반품/교환 신청 내역 </DIV> <br />
	<DIV style="display:inline-block; border:2px solid lightgray; padding:15px; width:850px; height:37px;">
		...
		<FORM style="display:inline-block" onsubmit="loadCancellationRequestTable(document.getElementById('start').value, document.getElementById('end').value)); return false;">
...
<jsp:include page="cancellationRequestTable.jsp" />